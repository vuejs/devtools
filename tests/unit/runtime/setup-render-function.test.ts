...
// https://github.com/vuejs/devtools/issues/376
// Classic: https://github.com/vuejs/devtools/issues/1295
...
describe('setup() returning a render function', () => {
  it('shows refs when setup returns a data object', async () => {
    // https://github.com/vuejs/devtools/issues/376
  ...
  it('shows expose() bindings when setup returns a render function', async () => {
    const snapshot = await snapshotNamedComponent(
      'RenderFnExposed',
      defineComponent({
        name: 'RenderFnExposed',
        setup(_, { expose }) {
          const count = ref(0)
          const state = reactive({ n: 1 })
          expose({ count, state })
  ...
  it('cannot recover closure-only setup refs that Vue never stores on the instance', async () => {
    const snapshot = await snapshotNamedComponent(
      'RenderFnClosedOver',
      defineComponent({
        name: 'RenderFnClosedOver',
        setup() {
          const count = ref(0)
          const state = reactive({ n: 1 })
          return () => h('div', `${count.value}:${state.n}`)
...
function setupKeys(snapshot: ComponentStateSnapshotMessage): string[] {
  return (snapshot.sections.find((section) => section.id === 'setup')?.entries ?? []).map(
    (entry) => entry.key,
  )
}
